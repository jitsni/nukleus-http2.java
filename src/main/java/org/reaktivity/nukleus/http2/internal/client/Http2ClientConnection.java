...
class Http2ClientConnection
{
    ...
    Http2ClientConnection(ClientStreamFactory factory, long routeId, long acceptReplyStreamId)
    {
    ...
    public boolean initConnection(BeginFW httpBegin)
    {
        ...
        RouteFW route = factory.resolveTarget(routeId, httpBegin.authorization(), headers);
        if (route == null)
        {
            return false;
        }
        ...
        this.nukleusStreamId = factory.supplyInitialId.applyAsLong(networkRouteId);
        MessageConsumer networkTarget = factory.router.supplyReceiver(nukleusStreamId);
        long correlationId = factory.supplyReplyId.applyAsLong(nukleusStreamId);

        // TODO not sure if using factory buffer for all connections is safe
        http2Writer = new Http2Writer(factory.writeBuffer, networkTarget, this, factory.bufferPool);

        factory.correlations.put(correlationId, this);

        //initiate connect stream
        factory.doBegin(networkTarget, networkRouteId, nukleusStreamId, 0L);
        factory.router.setThrottle(nukleusStreamId, this::handleConnectThrottle);
    ...
    void handleSettings(Http2FrameFW http2Frame)
    {
        ...
        {
            factory.settingsRO.forEach(this::decodeRemoteSetting);
            if (!connectionError)
            {
